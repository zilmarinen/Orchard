...
class FootpathInspectorCoordinator: Coordinator<FootpathInspectorViewController>, Inspector, MouseObservable {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            case .down(let position, _):
                ...
                guard let sceneView = self.sceneView,
                      ...
                      let hit = sceneView.hitTest(point: position.start, category: [.footpath, .footpathChunk]),
                      let tile = scene.meadow.footpath.find(tile: Coordinate(vector: hit)) else { return }

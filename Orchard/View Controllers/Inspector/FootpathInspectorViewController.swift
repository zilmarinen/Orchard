...
class FootpathInspectorViewController: NSViewController, Inspector {
    ...
    @IBAction func popUp(_ sender: NSPopUpButton) {
        ...
        case footpathTypePopUp:
            ...
            guard let layer = inspectable.layer, let footpathType = FootpathType(rawValue: sender.indexOfSelectedItem) else { return }
...
extension FootpathInspectorViewController {
    ...
    func update() {
        ...
        for footpathType in FootpathType.allCases {
            
            self.footpathTypePopUp.addItem(withTitle: footpathType.description)
            
            self.footpathTypePopUp.item(at: footpathType.rawValue)?.set(color0: footpathType.primaryColor.color, color1: footpathType.secondaryColor.color)
        }
        
        self.footpathTypePopUp.selectItem(at: layer.footpathType.rawValue)
    }
}

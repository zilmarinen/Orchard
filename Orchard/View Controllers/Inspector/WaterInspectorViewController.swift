...
class WaterInspectorViewController: NSViewController {
    ...
    weak var coordinator: WaterInspectorCoordinator?
    ...
    @IBAction func popUp(_ sender: NSPopUpButton) {
        ...
        case waterTypePopUp:
            ...
            guard let layer = inspectable.layer, let waterType = WaterType(rawValue: sender.indexOfSelectedItem) else { return }
...
extension WaterInspectorViewController {
    ...
    func update() {
        ...
        for waterType in WaterType.allCases {
            
            self.waterTypePopUp.addItem(withTitle: waterType.description)
            
            self.waterTypePopUp.item(at: waterType.rawValue)?.set(color0: waterType.primaryColor.color, color1: waterType.secondaryColor.color)
        }
        
        self.waterTypePopUp.selectItem(at: layer.waterType.rawValue)
